...
echo "[4/6] 配置 bypass permissions..."
...
# 安装 Git Hook（自动更新 Capability Registry）
echo "[4.5/6] 配置 Git Hook..."
GIT_HOOK_DIR="$SCRIPT_DIR/.git/hooks"
if [ -d ".git/hooks" ]; then
    HOOK_FILE=".git/hooks/post-commit"
    if [ ! -f "$HOOK_FILE" ] || ! grep -q "update-registry-on-commit" "$HOOK_FILE" 2>/dev/null; then
        cat >> "$HOOK_FILE" << 'HOOK_EOF'

# CHK: 更新 Capability Registry
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "$SCRIPT_DIR/update-registry-on-commit.sh" ]; then
    bash "$SCRIPT_DIR/update-registry-on-commit.sh" 2>/dev/null || true
fi
HOOK_EOF
        echo "      已配置 post-commit hook"
    else
        echo "      Git hook 已配置，跳过"
    fi
else
    echo "      不是 Git 仓库，跳过 hook 配置"
fi
