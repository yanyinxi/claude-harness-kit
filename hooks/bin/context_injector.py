...
def inject_plugin_memory(root: Path) -> str:
    """
    渐进式注入 CHK 记忆索引。

    遵循 Progressive Disclosure：
    - 第 1 层：注入 MEMORY.md（索引 + 每条一行核心要点）
    - 第 2 层：AI 按需 Read 具体 .md 文件读取完整内容
    """
    memory_index = root / "harness" / "memory" / "MEMORY.md"
...
def main():
    ...
    # 注入 CHK 插件记忆
    memory_context = inject_plugin_memory(root)
